...
static const ph7_expr_op aOpTable[] = {
  ...
  { { "(callable)", sizeof("(callable)") - 1 }, EXPR_OP_TYPECAST, 4, EXPR_OP_ASSOC_RIGHT, PH7_OP_CVT_CALLABLE },
...
static sxi32 ExprVerifyNodes(ph7_gen_state *pGen, ph7_expr_node **apNode, sxi32 nNode) {
  ...
  for (i = 0; i < nNode; ++i) {
    if (apNode[i]->pStart->nType & PH7_TK_LPAREN /*'('*/) {
      if (i > 0 && (apNode[i - 1]->xCode == PH7_CompileVariable || apNode[i - 1]->xCode == PH7_CompileLiteral || (apNode[i - 1]->pStart->nType & (PH7_TK_ID | PH7_TK_KEYWORD | PH7_TK_SSTR | PH7_TK_DSTR | PH7_TK_RPAREN /*')'*/ | PH7_TK_CSB /*']'*/ | PH7_TK_CCB /*'}'*/)))) {
...
static sxi32 ExprMakeTree(ph7_gen_state *pGen, ph7_expr_node **apNode, sxi32 nToken) {
  ...
  for (iCur = 0; iCur < nToken; ++iCur) {
    ...
    if (pNode->pOp && pNode->pOp->iPrec == 2 && pNode->pLeft == 0) {
      if (pNode->pOp->iOp == EXPR_OP_FUNC_CALL) {
        ...
        sxi32 iNest = 1;
        if (iLeft < 0 || apNode[iLeft] == 0 || (apNode[iLeft]->pOp == 0 && (apNode[iLeft]->xCode != PH7_CompileVariable && apNode[iLeft]->xCode != PH7_CompileSimpleString && apNode[iLeft]->xCode != PH7_CompileString)) || (apNode[iLeft]->pOp && apNode[iLeft]->pOp->iPrec != 2 /* postfix */)) {
          /* Syntax error */
        ...
        /* Link the node to the tree */
        pNode->pLeft = apNode[iLeft];
        if (pNode->pOp->iOp == EXPR_OP_ARROW /*'->'*/ && pNode->pLeft->pOp == 0 && pNode->pLeft->xCode != PH7_CompileVariable) {
          /* Syntax error */
          rc = PH7_GenCompileError(pGen, E_ERROR, pNode->pStart->nLine,
  ...
  for (iCur = 0; iCur < nToken; ++iCur) {
    ...
    if (pNode->pOp && pNode->pOp->iPrec == 3 && pNode->pLeft == 0) {
      if (iLeft >= 0 && ((apNode[iLeft]->pOp && apNode[iLeft]->pOp->iPrec == 2 /* Postfix */) || apNode[iLeft]->xCode == PH7_CompileVariable)) {
        /* Link the node to the tree */
        pNode->pLeft = apNode[iLeft];
        apNode[iLeft] = 0;
      }
