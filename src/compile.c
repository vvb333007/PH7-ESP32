...
static sxi32 GenStateFixGoto(ph7_gen_state *pGen, sxu32 nOfft) {
  ...
  for (n = nOfft; n < SySetUsed(&pGen->aGoto); ++n) {
    ...
    if (pLabel->pFunc != pJump->pFunc) {
      rc = PH7_GenCompileError(&(*pGen), E_ERROR, pJump->nLine, "Label '%z' is unreachable (out of the scope)", &pJump->sLabel);
      if (rc == SXERR_ABORT) {
        return SXERR_ABORT;
      }
    }
  ...
  for (n = 0; n < SySetUsed(&pGen->aLabel); ++n) {
    if (aLabel[n].bRef == FALSE) {
      /* Emit a warning */
      PH7_GenCompileError(&(*pGen), E_WARNING, aLabel[n].nLine,"Label '%z' is defined but not referenced", &aLabel[n].sName);
    }
  }
  return SXRET_OK;
}
 ...
 * Compiler calls this callback every time it finds a string literal (e.g. 'Hello!').
 * First, compiler tries to find if the same string was already registered as a literal
 * and fetch its index nIdx
 *
 * If it is registered before, then simple 'ld.c' instruction emitted with a string id.
 * Otherwise compiler creates new literal and installs its value.
 *
 * If literal is too long, it is NOT installed
 *
 * According to the PHP language reference manual:
 *
...
PH7_PRIVATE sxi32 PH7_CompileSimpleString(ph7_gen_state *pGen, sxi32 iCompileFlag) {
  ...
  if (pStr->nByte >= MAX_LITERAL_SIZE) {

    pStr->nByte = MAX_LITERAL_SIZE - 1;
    zEnd = &zIn[pStr->nByte];

    PH7_GenCompileError(&(*pGen), E_WARNING, pGen->pIn->nLine, 
        "String literal is too large, value is truncated.\n"
        "Maximum allowed string literal size is %d bytes (can be adjusted)\n",MAX_LITERAL_SIZE - 1);
  }
  ...
  for (;;) {
    ...
    while (zIn < zEnd && zIn[0] != '\\') {
      if (zIn[0] == '{' && &zIn[1] < zEnd && zIn[1] == '$') {
        break;
      } else if (zIn[0] == '$' && &zIn[1] < zEnd && (((unsigned char)zIn[1] >= 0xc0 || SyisAlpha(zIn[1]) || zIn[1] == '{' || zIn[1] == '_'))) {
        break;
      }
      zIn++;
    }
...
static sxi32 GenStateArrayNodeValidator(ph7_gen_state *pGen, ph7_expr_node *pRoot) {
  sxi32 rc = SXRET_OK;
  if (pRoot->pOp) {
    if (pRoot->pOp->iOp != EXPR_OP_SUBSCRIPT /* $a[] */ && pRoot->pOp->iOp != EXPR_OP_FUNC_CALL /* function() [Symisc extension: i.e: array(&foo())] */
      ...
      rc = PH7_GenCompileError(&(*pGen), E_ERROR, pRoot->pStart ? pRoot->pStart->nLine : 0,
                               "array(): Expecting a variable/array member/function call after operator '&'");
      if (rc != SXERR_ABORT) {
        rc = SXERR_INVALID;
      }
    }
...
static sxi32 PH7_CompileBacktic(ph7_gen_state *pGen, sxi32 iCompileFlag) {
  ...
  PH7_GenCompileError(&(*pGen), E_NOTICE, pGen->pIn->nLine,"Command line invocation is disabled for security reasons");
...
static sxi32 GenStateLoadLiteral(ph7_gen_state *pGen) {
  ...
  if (pStr->nByte == sizeof("NULL") - 1) {
    ...
    return SXRET_OK;
  } else if (pStr->nByte == sizeof("__LINE__") - 1 && SyMemcmp(pStr->zString, "__LINE__", sizeof("__LINE__") - 1) == 0) {
    ...
    PH7_VmEmitInstr(pGen->pVm, PH7_OP_LOADC, 0, nIdx, 0, 0);
    return SXRET_OK;
  } else if ((pStr->nByte == sizeof("__FUNCTION__") - 1 && SyMemcmp(pStr->zString, "__FUNCTION__", sizeof("__FUNCTION__") - 1) == 0) || (pStr->nByte == sizeof("__METHOD__") - 1 && SyMemcmp(pStr->zString, "__METHOD__", sizeof("__METHOD__") - 1) == 0)) {
    GenBlock *pBlock = pGen->pCurrent;
  ...
  if (pGen->pIn->nType & PH7_TK_OCB /* '{' */) {
    ...
    for (;;) {
      ...
      if (pGen->pIn->nType & PH7_TK_KEYWORD) {
        ...
        if (nKwrd == nKeywordEnd || (nKeywordEnd == PH7_TKWRD_ENDIF && (nKwrd == PH7_TKWRD_ELSE || nKwrd == PH7_TKWRD_ELIF))) {
...
static sxi32 PH7_CompileWhile(ph7_gen_state *pGen) {
  ...
  /* Update token stream */
  while (pGen->pIn < pEnd) {
    rc = PH7_GenCompileError(&(*pGen), E_ERROR, pGen->pIn->nLine, "Unexpected token '%z' in a 'while' statement", &pGen->pIn->sData);
    if (rc == SXERR_ABORT) {
      return SXERR_ABORT;
    }
    pGen->pIn++;
...
static sxi32 PH7_CompileDoWhile(ph7_gen_state *pGen) {
  ...
  if (pGen->pIn >= pGen->pEnd || pGen->pIn->nType != PH7_TK_KEYWORD || SX_PTR_TO_INT(pGen->pIn->pUserData) != PH7_TKWRD_WHILE) {
  ...
  /* Update token stream */
  while (pGen->pIn < pEnd) {
    rc = PH7_GenCompileError(&(*pGen), E_ERROR, pGen->pIn->nLine, "Unexpected token '%z' in 'do/while' statement", &pGen->pIn->sData);
    if (rc == SXERR_ABORT) {
      return SXERR_ABORT;
    }
    pGen->pIn++;
...
static sxi32 PH7_CompileIf(ph7_gen_state *pGen) {
  ...
  for (;;) {
    ...
    /* Update token stream */
    while (pGen->pIn < pEnd) {
      PH7_GenCompileError(&(*pGen), E_ERROR, pGen->pIn->nLine, "Unexpected token '%z' while compiling 'if'", &pGen->pIn->sData);
      pGen->pIn++;
    }
    pGen->pIn = &pEnd[1];
    pGen->pEnd = pTmp;
    ...
    if (nKeyID & PH7_TKWRD_ELSE) {
      pToken = &pGen->pIn[1];
      if (pToken >= pGen->pEnd || (pToken->nType & PH7_TK_KEYWORD) == 0 || SX_PTR_TO_INT(pToken->pUserData) != PH7_TKWRD_IF) {
        break;
      }
  ...
  /* Fix the false jump */
  GenStateFixJumps(pCondBlock, PH7_OP_JZ, PH7_VmInstrLength(pGen->pVm));
  if (pGen->pIn < pGen->pEnd && (pGen->pIn->nType & PH7_TK_KEYWORD) && (SX_PTR_TO_INT(pGen->pIn->pUserData) & PH7_TKWRD_ELSE)) {
      ...
      if (nRet && pFunc && (pFunc->iFlags & VM_FUNC_RET_TYPE)) {
        ...
        bNullable = ((pFunc->iFlags & VM_FUNC_RET_NULLABLE) != 0);
          ...
          if (pFunc->iFlags & PH7_TKWRD_INT) {
            ...
            nCvtOp = PH7_OP_CVT_OBJ; /* Generic class type */
...
static sxi32 PH7_CompileStatic(ph7_gen_state *pGen) {
  ...
  if (pGen->pIn >= pGen->pEnd || (pGen->pIn->nType & PH7_TK_DOLLAR) == 0 || &pGen->pIn[1] >= pGen->pEnd || (pGen->pIn[1].nType & (PH7_TK_ID | PH7_TK_KEYWORD)) == 0) {
    rc = PH7_GenCompileError(&(*pGen), E_ERROR, nLine, "Expected variable after 'static' keyword");
    if (rc == SXERR_ABORT) {
      return SXERR_ABORT;
    }
...
static sxi32 PH7_CompileNamespace(ph7_gen_state *pGen) {
  ...
  if (pGen->pIn >= pGen->pEnd || (pGen->pIn->nType & (PH7_TK_NSSEP | PH7_TK_ID | PH7_TK_KEYWORD | PH7_TK_SEMI /*';'*/ | PH7_TK_OCB /*'{'*/)) == 0) {
...
//    PH7_GenCompileError(pGen, E_NOTICE, pGen->pIn->nLine, "Next token is '%z'", pName);
     ...
     *  recognise `?` at the beginning of the type and save a special flag on corresponding sArg
     *  later, when performing Arg autocasting - do not autocast null values if `?` was used with type
     *   this is last push to make full support for nullable types.
     *  Do not include `?` into signature.
     */
...
choose_memobj:
        if (nKey & PH7_TKWRD_ARRAY) {
          ...
          PH7_GenCompileError(&(*pGen), E_WARNING, pGen->pIn->nLine,
                              "Invalid argument type '%z', compiled as ':mixed'",
                              &pIn->sData);
        }
      } else {
          ...
          if ((pClass->iFlags & PH7_CLASS_ENUM) != 0) {
             ...
             */
             ...
             goto choose_memobj;
              ...
              PH7_GenCompileError(pGen, E_WARNING, pGen->pIn->nLine,
                 "Typename '%z' is not yet known, assuming a class name.\n"
                 "(unlike classes, enums must be declared (or included) in the same translation unit)\n"
                 "\n"
,               pName);
...
//printf("0=%08x, 1=%08x\r\n",pIn[0].nType,pIn[1].nType);
    ...
    if (pIn < pEnd) {
      ...
      if (pIn < pEnd && (pIn->nType & PH7_TK_COMMA) == 0) {
        rc = PH7_GenCompileError(&(*pGen), E_ERROR, pIn->nLine, "Collecting function arguments: Unexpected token '%z'", &pIn->sData);
        return rc;
      }
    ...
    if (sArg.nType > 0) {
      ...
      /* PH7: No type is associated with this parameter which mean
      ...
      */
      ...
      /* PH8: No type is a 'mixed' type. mixed and no-types go here */
      ...
      while (pGen->pIn < pGen->pEnd) {
        ...
        if (pGen->pIn >= pGen->pEnd || (pGen->pIn->nType & PH7_TK_DOLLAR) == 0 || &pGen->pIn[1] >= pGen->pEnd
          ...
          if (zDup) {
            ...
            if (!got_this && pName->nByte == sizeof("this") - 1 && SyMemcmp((const void *)zDup, (const void *)"this", sizeof("this") - 1) == 0) {
              got_this = 1;
            }
...
static sxi32 GenStateCompileEnum(ph7_gen_state *pGen, sxi32 iFlags) {
  ...
  sxi32 rc;
  sxi32 nAutoValue = 0;
  sxu32 nKey = PH7_TKWRD_MIXED; // enum type is :mixed by default
...
static sxi32 PH7_CompileCatch(ph7_gen_state *pGen, ph7_exception *pException) {
  ...
  if (pGen->pIn >= pGen->pEnd || (pGen->pIn->nType & PH7_TK_LPAREN) == 0 /*(*/ || &pGen->pIn[1] >= pGen->pEnd || (pGen->pIn[1].nType & (PH7_TK_ID | PH7_TK_KEYWORD)) == 0) {
    ...
    rc = PH7_GenCompileError(pGen, E_ERROR, pToken->nLine,
                             "Catch: Unexpected token '%z',expecting a class name", &pToken->sData);
    if (rc == SXERR_ABORT) {
      return SXERR_ABORT;
    }
...
static sxi32 PH7_CompileTry(ph7_gen_state *pGen) {
  ...
  if (pGen->pIn >= pGen->pEnd || (pGen->pIn->nType & PH7_TK_KEYWORD) == 0 || SX_PTR_TO_INT(pGen->pIn->pUserData) != PH7_TKWRD_CATCH) {
    SyToken *pTok = pGen->pIn;
    if (pTok >= pGen->pEnd) {
      pTok--; /* Point back */
    }
...
static sxi32 GenStateCompileSwitchBlock(ph7_gen_state *pGen, sxu32 iTokenDelim, sxu32 *pBlockStart) {
  ...
  while (pGen->pIn < pGen->pEnd && (pGen->pIn->nType & (PH7_TK_SEMI /*';'*/ | PH7_TK_COLON /*':'*/)) == 0) {
    /* Unexpected token */
    rc = PH7_GenCompileError(&(*pGen), E_ERROR, pGen->pIn->nLine, "Switch: Unexpected token '%z'", &pGen->pIn->sData);
    if (rc == SXERR_ABORT) {
      return SXERR_ABORT;
    }
    pGen->pIn++;
...
static sxi32 PH7_CompileSwitch(ph7_gen_state *pGen) {
  ...
  pGen->pIn = &pEnd[1];
  pGen->pEnd = pTmp;
  if (pGen->pIn >= pGen->pEnd || &pGen->pIn[1] >= pGen->pEnd || (pGen->pIn->nType & (PH7_TK_OCB /*'{'*/ | PH7_TK_COLON /*:*/)) == 0) {
    pTmp = pGen->pIn;
    if (pTmp >= pGen->pEnd) {
      pTmp--;
    }
...
static sxi32 PH7_CompilePHP(
  ...
  pGen->pEnd = &pGen->pIn[SySetUsed(pTokenSet)];
#if 0
  for (pScript = pGen->pIn; pScript < pGen->pEnd; pScript++) {
    printf("token: %08x\n",(unsigned int)pScript->nType);
  }
#endif
...
PH7_PRIVATE sxi32 PH7_GenCompileError(ph7_gen_state *pGen, sxi32 nErrType, sxu32 nLine, const char *zFormat, ...) {
  ...
  /* Fatal errors. Keep collecting them */
  if (nErrType == E_ERROR || nErrType == E_PARSE) {
    ...
    if (pGen->nErr > PH7_ERR_LIMIT) {
      /* Error count limit reached */
        ...
        SyBlobFormat(pWorker, "%u Error count limit reached, aborting compilation\n", nLine);
  ...
  switch (nErrType) {
    case E_WARNING: zErr = "[W]"; break;
    case E_NOTICE: zErr = "[I]"; break;
    case E_USER_ERROR: zErr = "(E)"; break;
    case E_USER_WARNING: zErr = "(W)"; break;
    case E_USER_NOTICE: zErr = "(I)"; break;
    default:
      break;
  }
  rc = SXRET_OK;
