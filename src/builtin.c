        ...
        */
          ...
          if (xtype == PH7_FMT_FLOAT && exp + precision < PH7_FMT_BUFSIZ - 30) {
            ...
            if (exp < 0) *(zBuf++) = '0';         /* Digits before "." */
              ...
              for (; exp >= 0; exp--) *(zBuf++) = (char)vxGetdigit(&realvalue, &nsd);
...
static sxi32 AddTag(SySet *pSet, const char *zTag, int nByte) {
  ...
  /* Strip tags */
  for (;;) {
    while (zTag < zEnd && (zTag[0] == '<' || zTag[0] == '/' || zTag[0] == '?' || zTag[0] == '!' || zTag[0] == '-' || ((unsigned char)zTag[0] < 0xc0 && SyisSpace(zTag[0])))) {
      zTag++;
    }
    if (zTag >= zEnd) {
      break;
...
static sxi32 FindTag(SySet *pSet, const char *zTag, int nByte) {
  if (SySetUsed(pSet) > 0) {
    ...
    SyString sTag;
    while (zTag < zEnd && (zTag[0] == '<' || zTag[0] == '/' || zTag[0] == '?' || ((unsigned char)zTag[0] < 0xc0 && SyisSpace(zTag[0])))) {
      zTag++;
    }
    /* Delimit the tag */
    zCur = zTag;
...
static int PH7_builtin_str_pad(ph7_context *pCtx, int nArg, ph7_value **apArg) {
  ...
  zPad = s_Space; /* Whitespace padding */
