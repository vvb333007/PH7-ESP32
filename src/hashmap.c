...
static sxi32 HashmapInsert(
  ...
  if (pKey && pKey->iFlags & (MEMOBJ_STRING | MEMOBJ_HASHMAP | MEMOBJ_OBJ | MEMOBJ_RES)) {
    ...
    if (SXRET_OK == HashmapLookupBlobKey(&(*pMap), SyBlobData(&pKey->sBlob), SyBlobLength(&pKey->sBlob), &pNode)) {
...
PH7_PRIVATE sxi32 PH7_HashmapUnion(ph7_hashmap *pLeft, ph7_hashmap *pRight) {
  ...
  for (n = 0; n < pRight->nEntry; ++n) {
    ...
    if (pEntry->iType == HASHMAP_BLOB_NODE) {
      /* BLOB key */
      if (SXRET_OK != HashmapLookupBlobKey(&(*pLeft), SyBlobData(&pEntry->xKey.sKey), SyBlobLength(&pEntry->xKey.sKey), 0)) {
...
static const ph7_builtin_func aHashmapFunc[] = {
  ...
  /* array_unshift() is implemented in a stdlib as a native PHP function */
