...
#define GlogUpperToLower(A) \
  if (A < 0x80) { A = sqlite3UpperToLower[A]; }
/*
...
int uname(struct utsname *out) {
  ...
  out->sysname = "FreeRTOS-SMP";
#else
  out->sysname = "FreeRTOS";
#endif
  out->nodename = "localhost";
  out->release = xstr(tskKERNEL_VERSION_MAJOR);
  out->version = xstr(tskKERNEL_VERSION_MINOR) "." xstr(tskKERNEL_VERSION_BUILD);
#ifdef __XTENSA__
  // TODO: Get machine name from freertos/idf
...
PH7_PRIVATE void *PH7_StreamOpenHandle(ph7_vm *pVm, const ph7_io_stream *pStream, const char *zFile,
  ...
  if (use_include) {
    if (sFile.zString[0] == '/' ||
        (sFile.nByte > 1 && sFile.zString[0] == '.' && sFile.zString[1] == '/') || (sFile.nByte > 2 && sFile.zString[0] == '.' && sFile.zString[1] == '.' && sFile.zString[2] == '/')) {
...
static int csv_write_callback(ph7_value *pKey, ph7_value *pValue, void *pUserData) {
  ...
  n = 1;
  c2 = 0;
  if (SyByteFind(zData, (sxu32)nLen, pData->delimiter, 0) == SXRET_OK || SyByteFind(zData, (sxu32)nLen, pData->enclosure, &n) == SXRET_OK) {
    c2 = 1;
    if (n == 0) {
      c2 = 2;
    }
...
#endif //#if !defined(PH7_DISABLE_BUILTIN_FUNC) && !defined(PH7_DISABLE_DISK_IO)
